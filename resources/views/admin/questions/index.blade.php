<x-admin-layout>
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
            <h2 class="text-3xl font-bold text-slate-900 dark:text-white">Daftar Soal</h2>
            <p class="text-slate-600 dark:text-slate-400 font-medium">Kelola pertanyaan psikotest RIASEC Anda.</p>
        </div>
        <a href="{{ route('admin.questions.create') }}" 
            class="inline-flex items-center px-6 py-3 bg-blue-600 border border-transparent rounded-xl font-bold text-sm text-white uppercase tracking-widest hover:bg-blue-700 transition shadow-lg shadow-blue-500/20">
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50 dark:bg-slate-900/50 border-b border-slate-200 dark:border-slate-700">
                        <th class="px-6 py-4 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest">Kategori</th>
                        <th class="px-6 py-4 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest">Pertanyaan</th>
                        <th class="px-6 py-4 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest">Opsi</th>
                    ...
                    @if(count($questions) > 0)
                        @foreach($questions as $question)
                            <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition duration-150">
                                ...
                                <td class="px-6 py-4">
                                    <p class="text-sm font-medium text-slate-900 dark:text-slate-200 line-clamp-2 max-w-sm">{{ $question->question_text }}</p>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex items-center space-x-1">
                                        <span class="text-xs font-bold text-slate-500 dark:text-slate-400">{{ $question->options->count() }} Pilihan</span>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex justify-center items-center space-x-3">
                                        <a href="{{ route('admin.questions.edit', $question) }}" class="text-indigo-600 dark:text-indigo-400 hover:text-indigo-900 dark:hover:text-indigo-300 transition p-2 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 rounded-lg" title="Edit">
                        ...
                        <tr>
                            <td colspan="4" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    <p class="text-slate-500 dark:text-slate-400 font-medium">Belum ada soal yang ditambahkan.</p>
                                </div>
                            </td>
                        </tr>
                    @endif
