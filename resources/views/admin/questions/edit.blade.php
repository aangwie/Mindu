<x-admin-layout>
    <div class="mb-8">
        ...
        <h2 class="text-3xl font-bold text-slate-900 dark:text-white">Edit Soal</h2>
        <p class="text-slate-600 dark:text-slate-400">Perbarui pertanyaan psikotest RIASEC dan pilihan jawabannya.</p>
    </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <textarea id="question_text" name="question_text" rows="4" required
                                class="block w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-blue-500 focus:border-blue-500 transition dark:text-white">{{ old('question_text', $question->question_text) }}</textarea>
                        </div>
                        ...
                        <div>
                            ...
                            <select id="category" name="category" required
                                ...
                                @foreach(['R' => 'Realistic', 'I' => 'Investigative', 'A' => 'Artistic', 'S' => 'Social', 'E' => 'Enterprising', 'C' => 'Conventional'] as $key => $label)
                                    <option value="{{ $key }}" {{ old('category', $question->category) == $key ? 'selected' : '' }}>{{ $key }} - {{ $label }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                ...
                <div class="flex justify-end gap-4 mt-8">
                    ...
                    <button type="submit" class="px-10 py-3 bg-blue-600 border border-transparent rounded-xl font-bold text-sm text-white uppercase tracking-widest hover:bg-blue-700 transition shadow-lg shadow-blue-500/20">Perbarui Soal</button>
                </div>
            </div>
                        ...
                        <div class="p-4 bg-slate-50 dark:bg-slate-900/50 rounded-xl border border-slate-200 dark:border-slate-700 relative">
                            ...
                            <div class="space-y-4 pt-2">
                                <div>
                                    ...
                                    <input type="text" name="options[{{ $i }}][text]" required value="{{ old("options.{$i}.text", $option->option_text) }}"
                                ...
                                <div>
                                    ...
                                    <input type="number" name="options[{{ $i }}][point]" required min="1" max="5" value="{{ old("options.{$i}.point", $option->point) }}"
                                        class="block w-full px-3 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-sm focus:ring-blue-500 focus:border-blue-500 transition dark:text-white">
                                </div>
                            </div>
                        </div>
