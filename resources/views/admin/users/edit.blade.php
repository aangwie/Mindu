<x-admin-layout>
    <div class="mb-8">
        <div class="flex items-center gap-3 mb-2">
            ...
            <h2 class="text-3xl font-bold text-slate-900 dark:text-white">Edit User</h2>
        </div>
        <p class="text-slate-600 dark:text-slate-400 font-medium">Perbarui data user <strong>{{ $user->full_name }}</strong>.</p>
    </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div class="bg-white dark:bg-slate-800 p-6 md:p-8 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        <label for="password" class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">Password Baru <span class="text-xs text-slate-400 font-normal">(kosongkan jika tidak diubah)</span></label>
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" id="pob" name="pob" value="{{ old('pob', $user->pob) }}"
                                class="block w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-blue-500 focus:border-blue-500 transition dark:text-white">
                        </div>
                        <div>
                            <label for="dob" class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">Tanggal Lahir</label>
                            <input type="date" id="dob" name="dob" value="{{ old('dob', $user->dob ? \Carbon\Carbon::parse($user->dob)->format('Y-m-d') : '') }}"
                                class="block w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-blue-500 focus:border-blue-500 transition dark:text-white">
                        </div>
                    </div>
                    <div>
                        <label for="school_origin" class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">Asal Sekolah</label>
                        <input type="text" id="school_origin" name="school_origin" value="{{ old('school_origin', $user->school_origin) }}"
                            class="block w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-blue-500 focus:border-blue-500 transition dark:text-white">
                    </div>
                    <div>
                        <label for="current_school" class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">Sekolah Saat Ini</label>
                        <input type="text" id="current_school" name="current_school" value="{{ old('current_school', $user->current_school) }}"
                            class="block w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-blue-500 focus:border-blue-500 transition dark:text-white">
                    </div>
                    <div>
            ...
            <div class="lg:col-span-2 flex justify-end gap-3">
                ...
                <button type="submit" class="px-12 py-3 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-xl font-bold text-sm tracking-widest hover:bg-slate-800 dark:hover:bg-slate-100 transition shadow-xl shadow-slate-200 dark:shadow-none uppercase">Simpan Perubahan</button>
            </div>
        </div>
    </form>
</x-admin-layout>
