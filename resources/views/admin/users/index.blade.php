<x-admin-layout>
    <div class="mb-8">
        <h2 class="text-3xl font-bold text-slate-900 dark:text-white">Manajemen Siswa</h2>
        <p class="text-slate-600 dark:text-slate-400 font-medium">Lihat dan kelola data siswa yang terdaftar.</p>
    </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50 dark:bg-slate-900/50 border-b border-slate-200 dark:border-slate-700">
                        ...
                        <th class="px-6 py-4 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest">Kontak</th>
                    ...
                    @if(count($users) > 0)
                        @foreach($users as $user)
                            <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition duration-150">
                                ...
                                <td class="px-6 py-4">
                                    <p class="text-sm font-medium text-slate-700 dark:text-slate-300">NISN: {{ $user->nisn ?? '-' }}</p>
                                    <p class="text-xs text-slate-500 dark:text-slate-400">{{ $user->current_school ?? '-' }}</p>
                                </td>
                                <td class="px-6 py-4">
                                    <p class="text-sm font-medium text-slate-700 dark:text-slate-300">{{ $user->phone ?? '-' }}</p>
                                </td>
                                <td class="px-6 py-4">
                        ...
                        <tr>
                            <td colspan="4" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    <p class="text-slate-500 dark:text-slate-400 font-medium">Belum ada siswa yang terdaftar.</p>
                                </div>
                            </td>
                        </tr>
                    @endif
